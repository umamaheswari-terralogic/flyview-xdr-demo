...
const USERS = [
  { name: 'John Doe', email: 'john.d@acme.com', dept: 'Finance', risk: 87, riskCls: 'cr', mfa: 'TOTP', login: '09:14', status: 'ACTIVE', statusCls: 'ok' },
  { name: 'Robert Chen', email: 'r.chen@acme.com', dept: 'Finance', risk: 91, riskCls: 'cr', mfa: null, login: 'Yesterday', status: 'OFFBOARDING', statusCls: 'hi' },
  { name: 'Sarah Kim', email: 'sarah.k@acme.com', dept: 'Engineering', risk: 72, riskCls: 'hi', mfa: 'WebAuthn', login: '09:02', status: 'ACTIVE', statusCls: 'ok' },
  { name: 'Mike Ross', email: 'mike.r@acme.com', dept: 'Sales', risk: 44, riskCls: 'me', mfa: 'Push', login: '08:55', status: 'ACTIVE', statusCls: 'ok' },
  { name: 'Emma Clark', email: 'emma.c@acme.com', dept: 'HR', risk: 21, riskCls: 'ok', mfa: 'WebAuthn', login: '08:30', status: 'ACTIVE', statusCls: 'ok' },
]
...
export default function Identity() {
  return (
    <>
      <div className="kg k4">
        <MetricCard cls="ok" num="930" desc="Total managed users" label="Directory" foot="<b>+3</b> this week" icon={Icons.user} />
        ...
        <MetricCard cls="ok" num="94%" desc="MFA enrollment rate" label="MFA Coverage" foot="6% on TOTP only" icon={Icons.lock} />
      </div>
      ...
      <div className="card" style={{ marginBottom: 18 }}>
        <div className="card-h">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <div className="seg"><button className="on">All</button><button>High Risk</button><button>Offboarding</button></div>
            <button className="btn p">Invite user</button>
          </div>
        </div>
        <table>
          <thead><tr>{['User', 'Dept', 'Risk Score', 'MFA', 'Last Login', 'Status', ''].map(h => <th key={h}>{h}</th>)}</tr></thead>
          <tbody>
            {USERS.map(u => (
              <tr key={u.email}>

    ...
    <>
      ...
      <div className="g3">
        <div className="card"><div className="card-h"><h3>Flow anomalies</h3></div><div className="card-b">
          {flowAnomalies.slice(0, 4).map(f => (
            <div key={f.id} className="row">
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12.5, fontWeight: 500 }}>{f.type}</div>
                <div className="mono" style={{ fontSize: 11, color: 'var(--txt3)' }}>{f.src} → {f.dst}</div>
              </div>
              <span className={`b ${f.sevCls}`}><i />{f.severity}</span>
            </div>
          ))}
        </div></div>

        <div className="card"><div className="card-h"><h3>Config changes (7d)</h3></div><div className="card-b">
          {configChanges.slice(0, 4).map(c => (
            <div key={c.id} className="row">
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600 }}>{c.device}</div>
                <div style={{ fontSize: 11, color: 'var(--txt3)' }}>{c.changeType}</div>
              </div>
